...
| 2026-05-27 | Substrate — self-hosted Llama 3.3 70B on Hetzner is the AI safety net | Craig: "If AI went down tomorrow how would we match the competition." The Substrate is the answer. A single Hetzner H100 box running vLLM serving Llama-3.3-70B-Instruct on an OpenAI-compatible API. Registered in `src/lib/llm-provider.ts` as the `selfhosted` provider with two model IDs (`zoo-llama-3.3-70b` balanced + `zoo-llama-3.1-8b` fast). Slots in **last** in the failover chain (Claude → OpenAI → Gemini → Substrate) so it's a rainy-day backstop, not a primary. Activated by setting `SELFHOSTED_LLM_URL` + `SELFHOSTED_LLM_KEY` in Vercel — the moment those exist, builder/edit/video pipelines automatically gain a Anthropic-independent fallback at fixed ~€2k/mo cost regardless of usage. Deployment runbook is at `docs/substrate-deploy.md`. Admin probe at `GET /api/admin/substrate-health`. **Do not make the Substrate the primary** without Craig's explicit override — its purpose is to make us robust to provider outages, not to replace Anthropic for quality-critical builds. |
