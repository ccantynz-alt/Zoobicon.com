...
## Last updated: 2026-06-13 (OLD V1 BUILDER DELETED + whole-page AI engine on Opus with live "watch it build" streaming. THE BUILDER IS THE V2 ENGINE NOW — `/builder` rebuilt from scratch: clean-white ZOOBICON BOLD chrome on the proven server-render pipeline. Full repo audit (3 parallel agents) confirmed 100% of traffic hit the broken 3,440-line V1 (Sandpack/EscapeHatch chain, 10+ silent blank-screen paths) while the reliable V2 engine sat unlinked at /v2. V1 DELETED 2026-06-13 (Craig: "delete the old builder… we're moving forward not backwards") — `/builder/classic` + 23 orphaned V1-only components removed, 11,596 lines; `/v2` still redirects to `/builder`. The whole-page AI engine (`src/lib/v2/ai-site.ts`, Opus 4.7, streaming "watch it build" + prompt caching) is now the ONLY builder.) | Build: GREEN | Scope: AI WEBSITE BUILDER ONLY (Rule 32) | Branch: claude/zen-wright-wvvzq2

**THE /BUILDER REBUILD — SESSION 2026-06-12.** Craig: *"the builder has never worked for months… I want it nice white and clean… the most polished interactive design possible."* Root cause from the 3-agent audit: every homepage/SEO/share link routes to `/builder` (V1) — 3,440 lines, 80+ useState hooks, browser-side code execution (Babel/Sandpack/EscapeHatch), and 10+ silent failure paths that blank the preview with no message. The reliable V2 engine (server-rendered sections, no-AI floor, never-blank, 16/16 tests) had ZERO links pointing at it. Fix = CLAUDE.md's own documented next action ("port streaming+live+edit onto the main /builder page"): **`/builder/page.tsx` is now a ~700-line clean-white builder on the V2 engine** — `--zb-*` tokens only (white surfaces, ink text, ONE red accent, Jakarta display, zero dark chrome), same left-panel/right-preview layout. Features: streaming build via `/api/v2/build/stream` (one-shot `/api/v2/build` fallback), live hot-swap preview iframe, 3-step progress UI, conversational refine chat (`/api/v2/edit`, disabled mid-build — fixes the V1 edit/build race), desktop/tablet/mobile device toggle, **Download site** + **Open in new tab** (exports the LIVE iframe DOM with the streaming runtime stripped, so refine edits are included), generation-ID guard against stale-stream state writes, `?prompt=` auto-build preserved (homepage hero/SEO/share/upgrade funnels unchanged). Old V1 DELETED 2026-06-13 (Craig approved) — `/builder/classic` (3,440 lines) + 23 orphaned V1-only components (Sandpack/EscapeHatch/PrewarmFrame/PromptInput/ChatPanel-era panels) removed, 11,596 lines total; build stayed green, proving the new builder shares none of it. `/v2` → redirect `/builder`.

**ZOOBICON BOLD MIGRATION — SESSION 2026-06-09 (this branch).** Rule 37 (the Klaviyo-beating direction Craig set) was only live on 3 files (homepage + HeroBuilder + globals.css); the chrome wrapping every page was still a mishmash of three retired design eras. This session migrated the three highest-visibility surfaces onto the `--zb-*` system and — for the first time — **screenshot-verified the result in a real browser before pushing** (headless Chromium against `next start`; no more guessing what shipped looks like): (1) **SiteNavigation.tsx** full rewrite — was a light-cream bar with *white* text links (invisible), Playfair serif logo, lime-gradient CTA with white text + gold border; now a confident near-black Klaviyo-style stratum: lime square Z mark, bold Jakarta wordmark, white links, ONE lime pill CTA, ink mega-menu + mobile menu, 72px tall to match every page's `pt-[72px]`. (2) **SiteFooter.tsx** full rewrite — dark statement footer: `zb-display` "Build the thing. Today." CTA band with lime accent, link columns on ink, lime eyebrows, trust badges, three-domain signature. (3) **pricing/page.tsx** full rewrite — was the old dark "cinematic" Fraunces-italic/champagne page; now bright `zb-bright` canvas, `zb-display` hero with `zb-mark` highlight, ink/lime billing toggle, 4 plan cards (Pro = dark `zb-card-dark` featured with lime CTA), lime check trust strip, dark FAQ statement band. Also fixed Rule 35-violating copy (FAQ claimed Haiku in the build pipeline; now correctly says Sonnet 4.6 for generation, Opus for deep builds). **Still to migrate onto `--zb-*`:** builder UI chrome, /upgrade, auth, compare pages, generators/agencies/marketplace. **NEXT ACTION:** migrate /upgrade + /compare pages onto `--zb-*`, then the builder chrome; keep screenshot-verifying every visual change before push (script pattern: scroll-through then capture, `/tmp/pw/shot2.js` this session).
...
All gates green (build 245/245, critical-code-guard clear, tsc clean on changed files, 13/13 v2 tests). **NEXT ACTION (refreshed 2026-06-12):** (a) Craig reviews the Vercel preview of the new `/builder` — on his yes, DELETE `/builder/classic` (3,440-line V1) + the now-orphaned V1-only components (SandpackPreview/EscapeHatchPreview/PrewarmFrame/PromptInput/etc. — verify orphanhood first); (b) **full-stack / Crontech pressure-test** — verify the Crontech API can provision auth+DB+forms, then wire "describe a business → working app" into the builder; (c) global/multi-section edits ("make everything use a blue accent"); (d) ~~click-to-select a section in the preview~~ ✅ DONE 2026-06-12 — "Point & edit" button in the preview chrome toggles pick mode (crosshair + red outline on hover inside the iframe, capture-phase click so section buttons don't fire), picked section becomes a chip on the refine chat and `/api/v2/edit` accepts `targetIndex` to skip instruction routing; (e) deploy step (Crontech) + domain-in-checkout on the new builder's done state; (f) **developer-grade tier** (Craig vision 2026-06-12: "preview while coding, run bash, SSH with saved credentials per user") — the locked path is Crontech-provisioned per-user workspaces (2026-05-04 decision: no WebContainers license / no e2b without Craig's override). BLOCKED on CRONTECH_PAT (still unset/mocked) + Crontech workspace API. Per-user SSH credentials, if offered, must be vault-encrypted at rest, short-lived keys preferred — never raw passwords in our DB.
...
|---------|--------|-----------|------------|----------------|
| **AI Website Builder** (THE PRODUCT) | WORKING — V2 engine at /builder (2026-06-12) | `src/app/builder/page.tsx` (clean-white V2 UI), `src/app/api/v2/build/stream`, `src/lib/v2/render-page.ts`, `src/lib/v2/edit-section.ts` | Server-rendered streaming build (never-blank, no-AI floor), live React hydration, conversational refine, device preview, HTML export. V1 parked unlinked at `/builder/classic` | Craig approval to delete /builder/classic + deploy step (Crontech) + Stripe products + domain-buy in finalize flow |
...
37. **ZOOBICON BOLD — THE WEBSITE IS THE SALES DRIVER (overrides Rule 29)** (added 2026-06-09). Craig's words: *"how do we make zoobicon look like [klaviyo.com] ... we should be able to make something even better than this. This is what sell. We don't have to stick with the gold, you don't need to worry about Rule 29 I override that. I want something better, I'm sick of looking at that broken site that hasn't worked in months ... we want to sell this product and we need the website to be the driver."* The reference Craig admired is **klaviyo.com** (bold, confident, high-contrast — bright warm-white canvas + near-black statement sections + one saturated accent + big bold display type). The mandate is **not to copy Klaviyo but to beat it.** **The locked direction — ZOOBICON BOLD:** (a) **Palette** — a bright warm-white canvas `--zb-bg #faf9f5`, near-black statement sections `--zb-ink #0b0b0d`, and ONE electric-lime signature accent `--zb-accent #d4f24e` (with `--zb-accent-ink #161d05` for legible text on lime). No gold lock, no Playfair, no champagne. The accent is used as: marker-highlight on display words over light (`.zb-mark`), lime text/glow over dark, and lime fills on CTAs. (b) **Type** — bold display in **Plus Jakarta Sans 800** (already loaded in `layout.tsx`), tight `-0.04em` tracking, via `.zb-display`. Body stays Inter. (c) **Rhythm** — alternate bright content sections with near-black statement bands for confident contrast (the Klaviyo punch). (d) **Tokens + classes** live at the TOP of `globals.css` under the `--zb-*` namespace and `.zb-*` classes, deliberately separate from the legacy `--paper`/`--ink`/`--gold` system so the old editorial-light override layer at the bottom of the file can never repaint them. **Shipped this session (homepage as proof-of-concept):** new `--zb-*` design system in `globals.css`; rewrote `src/components/HeroBuilder.tsx` as a near-black statement hero (lime accent, big Jakarta headline) whose primary CTA **routes reliably to `/builder`** instead of running the inline SSE stream that has been failing for months — reliability over theatre, because the homepage's job is to convert; rewrote `src/app/page.tsx` with the bright/dark section rhythm (feature bento, dark how-it-works, founder's note, pricing, dark final CTA). **Still to migrate (next sessions, page-by-page onto `--zb-*`):** the builder UI, pricing page, `/upgrade`, auth, SEO/compare pages, nav + footer. Each migration replaces `text-white`/`--gold`/`--paper` usage with the `--zb-*` system and removes reliance on the override layer; when the last page is migrated, the override layer and legacy tokens can be deleted. **This is the floor, not the ceiling (Rule 0A):** the bar is "more premium and higher-converting than klaviyo.com," re-evaluated as competitors ship. Only Craig reverses this (same authority that set it). **AMENDED 2026-06-09 (same day, Craig): LIGHT-FIRST IS MANDATORY.** Craig's words after seeing the dark-heavy version: *"that whole cyberpunk thing just screams crazy it doesn't install trust in people this is nothing like the screenshots that I sent you ... we're trying to sell them product here ... it needs to be 100K site."* The Klaviyo reference is overwhelmingly LIGHT — dark is ONLY the slim nav bar and at most ONE flat statement band per page. Locked corollaries: (1) the page canvas is always `--zb-bg` bright warm-white; (2) **no radial lime glows, no dot-grids, no neon-on-black ambience — ever** (that is what read as cyberpunk); (3) dark bands are FLAT near-black with white type and at most one lime word or one lime button; (4) primary CTAs on light surfaces are INK pills (`.zb-btn-ink`), lime fills (`.zb-btn`) are reserved for the dark nav CTA and dark bands; (5) the legacy `--paper`/`--ink`/`--gold` tokens are repointed at the LIGHT palette (gold family = deep olive-lime, legible on white) so every unmigrated token-driven page renders light — the 2026-06-09 "global dark flip" is reversed and must not be reintroduced. **AMENDED #2 2026-06-09 (same day, Craig): THE ACCENT IS RED — LIME/YELLOW IS BANNED EVERYWHERE.** Craig's words: *"a bit of lime, a bit of yellow … this is still an entry-level developer build … I even sent you screenshots — that's the output I wanted."* A full sweep replaced every lime literal in src/ + tailwind.config.ts. The accent is now Klaviyo-class red `--zb-accent #e8402b` (hover `#ef5440`) with WHITE text on red (`--zb-accent-ink #ffffff`); the legacy `--gold` family resolves to deep reds (`#cf3a26`, text `#b32d1c`). `.zb-mark` is NO LONGER a marker highlight — it renders a plain red word (highlighter blocks read entry-level). `.zb-display` softened to weight 700 / -0.03em. The hero puts THE PRODUCT ON STAGE — an app-frame mockup of the builder generating a real site (Klaviyo's "show the product" pattern); the trust marquee is a flat RED brand band with white text. **Structural guard:** a `ZB DARK-SURFACE EXEMPTION` block must remain the LAST block in `globals.css` — the light override layer repaints `.text-white` utilities AND inline white styles (attribute selectors) to ink, so genuinely-dark zb surfaces (`.zb-dark`, `.zb-nav`, `.zb-card-dark`, `.zb-band`) need scoped white restored there. Any new override rules go ABOVE that block, never below.

38. **THE WHOLE-PAGE AI ENGINE + OPUS AS ITS GENERATION MODEL (amends Rules 4/17/35 for this engine only)** (added 2026-06-13). Craig, after months of templated output and a screenshot of the live builder giving a *pizza shop* a software-developer hero with a code snippet: *"Why can't we have that then… months… I feel like crying"* and *"I was only defaulting to Sonnet because I had such a bad experience with Opus but you've proved it wrong — if we can get this thing working then I'll stick with Opus."* **Root cause of "every build looks templated":** the builder stitched ~10 fixed registry components and only let the model rewrite their COPY — so a pizza shop got a SaaS dark-hero with a code editor, because that was the only "dark hero" in the drawer. The model never got to design. **The fix (this session):** a whole-page engine — `src/lib/v2/ai-site.ts` (`generateAiSite` + `editAiSite`) → `/api/v2/build/ai` + `/api/v2/edit/ai`. The model DESIGNS and writes a complete, self-contained HTML page (its own layout/palette/type/motion) for the specific business — the approach Lovable/Bolt/v0/Manus use. Reliability is preserved/improved: the iframe just DISPLAYS finished HTML (no in-browser transpile/module-load → no WebKit/iPad blank-preview class of bug), and if AI generation fails the route falls back to the deterministic registry render (never blank). The builder tries `/api/v2/build/ai` first; the old streaming engine stays as the safety net. **Model decision:** `GENERATION_MODEL` = `claude-opus-4-8` for BOTH build and edit (fixed 2026-06-13 from the stale `claude-opus-4-7`, which the live API rejected → the builder silently fell back to the OLD template engine every time, the "still looks templated + 244s slow" bug Craig caught in production; the engine now tries Opus then auto-falls-back to Sonnet 4.6 so a bad/stale ID can never silently drop a build to a template again) (Craig's call — his bad Opus experience was the fragile V1 runtime + the slot cage, not the model; with design freedom Opus produces the bespoke "$100K" output). `callLLMWithFailover` fails over Opus → Sonnet 4.6 → other providers, so availability never hard-fails a build. **Scope:** amends Rules 4/17/35 for the whole-page engine ONLY. **Rule 35's Haiku ban stands absolutely — Haiku is NEVER used for generation.** The legacy slot/registry routes (`react-stream`, `slot-stream`, `/api/v2/build` copy-tailoring) keep Sonnet per Rule 35 and are now the fallback path. Don't flip this engine back to Sonnet without Craig (Rule 21); a future cost/speed tier may *add* a Sonnet option, but Opus is the chosen default. Shipped on branch `claude/sweet-meitner-5tay6h` (PR #445). **Live-proof caveat:** the build sandbox has no `ANTHROPIC_API_KEY`, so the AI path is verified by structure + the identical-call demos, not run end-to-end here; the real visual proof is on the Vercel preview / production where the key exists.
...
|---|-------|-------|---------------|
| 44 | **Built sites had dead hero CTAs + wrong-industry photos; homepage band listed suppliers; Z-mark experiment** (Craig 2026-06-12 screenshots: bakery hero "Order Online Now"/"See What's Baking" did nothing; bakery showed analytics dashboards, server racks, beaches/forests; *"the red bar… is displaying all our competition as opposed to gatetest.ai, vapron.ai, gluecron.com, alecrae.com"*; *"the Z… should be red and the background… white but slightly different shade"*). Root causes: (a) many registry components render CTAs as bare `<button>` with no onClick and no href; (b) components ship fixed Unsplash IDs (SaaS dashboards/nature) regardless of business. | 2026-06-12 | **Source-level transforms** in render-page.ts (run on component CODE so static render, live hydration, export, and edits all agree — an HTML-only patch would be undone on hydrate): `swapImagesForIndustry()` swaps every Unsplash ID for the curated industry pool in stock-images.ts (new `industryPhotoPool()` bridge; deterministic by section index; randomuser avatars untouched) and `activateCtas()` converts behaviour-less CTA buttons into real anchors via an intent map (order→#contact/#ecommerce, menu/view→#features, book→#contact, pricing→#pricing, start/trial→#cta; onClick + submit buttons never touched). Wired into stream route (both phases) + renderFromRegistry. Verified e2e: bakery build → 8/8 images from the restaurant pool, hero CTAs "Reserve a Table"→#contact / "View Our Menu"→#features live in static html AND the hydration module, 0 dead CTA buttons. **Homepage red band** now lists the family — Vapron (+4 feature entries), GateTest, Gluecron, AlecRae — never suppliers/competitors. **Z-mark experiment** shipped: red Z on off-white tile (nav 21px, builder, footer) replacing white-Z-on-red. +3 tests (23/23 v2). |
| 43 | **Footer/nav links in built sites errored or did nothing** (Craig 2026-06-12: *"all links in the footer weren't working they would come back as errors maybe not linked"*). Three defect classes: (a) components link to anchors (#features/#pricing/#faq) but sections never had ids → clicks did nothing; (b) route-style hrefs (/about, /pricing) 404 on a single-page site and, inside the srcdoc preview iframe, navigated it to an invalid URL → error page; (c) clicking an external link navigated the preview iframe away entirely. | 2026-06-12 | `normalizePageLinks()` in render-page.ts (synonym-mapped: /services→#features, /plans→#pricing, etc.; unmatched internal links defused to inert "#"; external/mailto/tel untouched) + first-of-category sections now carry their category as an anchor id (with a guard against components that already self-id) — applied in the stream route (streamed sections AND the done artifact), renderFromRegistry, and /api/v2/edit so refined sections keep working links. Preview shell adds a capture-phase link safety net: externals open in a new tab, known anchors smooth-scroll, everything else inert — the preview iframe can never navigate into an error page. Verified e2e against `next start`: 10/10 sections, ids navbar→footer unique, every anchor href resolves, 0 route hrefs. +4 tests (20/20 v2). |
| 42 | **"The builder has never worked for months" — V1 was the only builder users could reach** (Craig 2026-06-12: *"I don't care about what's being built so far because it's been absolute rubbish… I want it nice white and clean… the most polished interactive design possible"*). 3-agent audit: every link (homepage hero, SEO pages, share forks, generators) routes to `/builder` = the 3,440-line V1 with browser-side code execution and 10+ silent blank-screen failure paths (empty-file-tree blank preview, stale throttle closure mixing two builds' files, silently-swallowed validation errors, edit/build race, EscapeHatch 3-5s cold load read as "failed"). The reliable V2 engine had zero inbound links. | 2026-06-12 | Rebuilt `/builder/page.tsx` from scratch (~700 lines) on the V2 server-render pipeline with clean-white ZOOBICON BOLD chrome (`--zb-*` only — white surfaces, ink, red accent, Jakarta display, no dark/cyberpunk). Streaming build + live hydration + refine chat (blocked while building — kills the race) + device toggle + Download/Open export of the live DOM (edits included, streaming runtime stripped) + generation-ID stale-stream guard + `?prompt=` auto-build (all funnels keep working). V1 moved intact+unlinked to `/builder/classic` pending Craig's delete approval (§2 item 6). `/v2` → redirect. **Bonus root cause killed:** Next.js App Router aliases `react` to the Server-Components subset inside route handlers — hooks don't exist there — so EVERY registry component calling `React.useState` (navbar-dark, testimonials-carousel, faq-accordion…) silently vanished from production pages while vitest (full React) passed. `render-page.ts` now supplies static-render hook semantics (initial state, no effects) when hooks are missing: SaaS build went 7/10 → **10/10 sections**, navbar restored. Also un-silenced the section-failure catches (console.warn) so this class of bug can't hide again. Build GREEN, icon check clean, tsc clean on changed files, 16/16 v2 tests, end-to-end verified against `next start`: real streaming builds (2 prompts, 10/10 + 8/8 sections, 0.6s, valid full-page artifact with nav/FAQ/testimonials), `/v2` redirect, edit-endpoint error contract, `/builder` 200. (Screenshot verification impossible this session — sandbox network policy blocks browser downloads; Craig reviews the Vercel preview.) |
