...
## Last updated: 2026-06-05 | Build: GREEN (245/245) | Scope: AI WEBSITE BUILDER ONLY (Rule 32) | Branch: claude/website-builder-audit-clone-XUKDZ

**NEXT ACTION:** Preview-crash root cause KILLED — the "Preview failed" / `@about:srcdoc` blank-screen Craig has hit for months was a fault-isolation gap: ANY one of the ~10 generated components throwing (at module-eval OR render time) crashed the WHOLE preview. Shipped 3-layer defence: (1) EscapeHatchPreview isolation pass — each section module test-imported up front, failures swapped for null-render stubs so the entry rebuilds clean; (2) per-section `<SectionBoundary>` baked into every generated App.tsx (react-stream + slot-stream) catching render errors → one bad section degrades invisibly, the other 9 render perfectly; (3) top-level RootBoundary showing a friendly card instead of a cryptic stack. Also surfaced the URL clone-and-upgrade funnel on the homepage hero ("Already have a website? Paste the URL — we'll rebuild it better" → /upgrade) — the feature was fully built (/upgrade + /api/upgrade/analyze + url-extractor) but buried behind the /import nav link, so visitors with an existing WordPress/Wix/any-platform site had no entry point. Next: verify on Vercel with a real build that one intentionally-broken component now degrades instead of blanking; then builder left-panel white redesign + MCP integration.
...
|---|-------|-------|---------------|
| 39 | **"Preview failed" blank-screen crash — THE months-long bug** (Craig 2026-06-05, screenshot: all 10 components generate green, then `@about:srcdoc:96:38` / `module code@about:srcdoc:345:7` → blank). Root cause: the EscapeHatchPreview rendered all generated components in ONE React tree with zero fault isolation. Sonnet customises registry components into TSX non-deterministically, so any single component with a module-evaluation error (bad top-level expression, destructure of a stubbed package) OR a render error (`undefined.map`, bad icon, hook misuse) threw → the whole `App()` import/render threw → final `catch` fired "Preview failed" with the cryptic srcdoc stack → blank screen. One flaw in 1 of 10 components = total failure. | 2026-06-05 | **3-layer defence-in-depth.** Layer 1 (`EscapeHatchPreview.tsx`): extracted `compileModule(path)` helper; transpile failures on `components/*` now stub to a null-component instead of aborting the whole build; added an ISOLATION PASS that `Promise.allSettled`-imports every section module up front and swaps any that throw at evaluation for a null-render stub, then rebuilds the entry against the stubbed URLs (Babel cache-hot, cheap). Layer 2 (`component-registry/index.ts` `buildAppFile` + the first App builder, and `slot-stream/route.ts` `buildSlotAppFile`): every section is now wrapped in a `<SectionBoundary name="…">` class component (`getDerivedStateFromError` → renders `null`) baked into the generated App.tsx — a render error in one section degrades invisibly while the other sections render perfectly; far more professional on a live site than an error box. Layer 3 (`EscapeHatchPreview.tsx`): a top-level `RootBoundary` wraps `<App/>` at render so anything that escapes shows a friendly "This section hit a snag — the rest of your site is fine" card instead of a blank screen / window.onerror. Net effect: the preview is now effectively un-crashable — worst case is one missing/degraded section, never a blank page. Build GREEN 245/245. Also surfaced the URL clone funnel on the homepage hero (was built but buried behind /import): `HeroBuilder.tsx` now shows "Already have a website? Paste the URL — we'll rebuild it better" → `/upgrade`. Branch `claude/website-builder-audit-clone-XUKDZ`. |
