...
## Last updated: 2026-06-09 (ZOOBICON BOLD rebrand — Rule 29 overridden) | Build: GREEN (248 pages) | Scope: AI WEBSITE BUILDER ONLY (Rule 32) | Branch: claude/gracious-mayer-qovnk1

**NEXT ACTION (ZOOBICON BOLD rollout, Rule 37):** Craig overrode Rule 29 (the gold/cream editorial-light look) on 2026-06-09 — *"I want something better, I'm sick of looking at that broken site."* Shipped the new **ZOOBICON BOLD** identity as a homepage proof-of-concept: new `--zb-*` design system in `globals.css` (bright warm-white `#faf9f5` + near-black `#0b0b0d` statement sections + electric-lime `#d4f24e` accent + Plus Jakarta Sans 800 display), a rewritten near-black hero (`HeroBuilder.tsx`) whose CTA now **routes reliably to `/builder`** instead of the flaky inline SSE stream, and a rewritten `page.tsx` with bright/dark section rhythm. Build green (248 pages), icons clean. **Next:** migrate the rest of the site onto `--zb-*` page-by-page — builder UI first (it's what converts after the homepage), then pricing, `/upgrade`, nav + footer, auth, SEO/compare pages. Each migration drops `text-white`/`--gold`/`--paper` usage; when the last page is done, delete the legacy override layer + tokens. Then verify the `/builder` path actually generates end-to-end (Craig: "hasn't worked in months") — the homepage now looks the part, the builder must deliver.

**NEXT ACTION (V2 speed + quality):** Builder V2 (`/v2`) now builds **progressively** — `/api/v2/build/stream` (SSE) renders the complete $100K-registry page from base components instantly (~1-2s, no AI, streamed section-by-section into the iframe), then AI-tailors each section's copy in the background and hot-swaps it in place via postMessage (Tailwind Play CDN restyles inserted nodes automatically). No more staring at a spinner while every 4000-token rewrite finishes — first paint beats Bolt's 3-5s. Plus theme-aware **serif display headings** (Playfair, already loaded but previously unused) for editorial/portfolio/restaurant/professional/agency industries — the audit's "$100K serif pairing" gap. The one-shot `/api/v2/build` stays as a graceful fallback if SSE can't be established. All gates green (build 245/245, critical-code-guard clear, tsc clean on changed files, 8/8 v2 tests). **Next:** (a) extract-text-slots copy fill (return a compact JSON of text values instead of the whole 4000-token component → cuts AI latency ~5-10x AND removes the risk of the model breaking JSX); (b) per-business accent palette (components currently all share violet/indigo — derive an industry-appropriate accent and thread CSS vars through the registry); (c) wire deploy/save off `result.html`; (d) port the progressive-stream UX onto the main `/builder` page once proven on `/v2`.
