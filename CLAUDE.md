...
## Last updated: 2026-06-10 (v5: BUILDER REPLACED with server-rendered V2 engine + ZB BOLD chrome (Rule 38), 38 orphaned components deleted, Crontech→Vapron rename in copy (Rule 39), brand band = own promises only, srcdoc /vendor/ hotfix LIVE in production via PR #439. Homepage v4 Klaviyo architecture + new builder ON BRANCH awaiting Craig's preview approval — do NOT merge without his yes) | Build: GREEN | Scope: AI WEBSITE BUILDER ONLY (Rule 32) | Branch: claude/ai-builder-platform-la5d1g
...
38. **THE BUILDER IS SERVER-RENDERED — IN-BROWSER CODE-EXECUTION SANDBOXES ARE RETIRED** (added 2026-06-10). Craig's words: *"We need to completely change the design and architecture of this builder I don't want to see it any more it's been faulty for far too long"* + *"is sandbox the best way to go?"* Answer locked in: NO. Client-side compilation (Babel-in-browser, importmaps, blob modules — the Sandpack/EscapeHatch architecture) was the root cause of months of "Preview failed" crashes, including the final one (srcdoc iframes can't resolve root-relative /vendor/ URLs — only reproducible in production). `/builder` is now the V2 server-rendered engine: `/api/v2/build/stream` renders sections server-side and streams finished HTML into a plain iframe; `/api/v2/edit` hot-swaps one section per chat instruction. The browser NEVER executes generated code. This is how Lovable/v0 do previews. Deleted in the same session: the 3,440-line legacy builder page and 38 orphaned builder-era components (PromptInput, SandpackPreview, WebContainerPreview, PrewarmFrame, DeployModal, GitHubSyncPanel, etc. — Craig: "if it's old and we're not using it then please delete it otherwise on the next container session one of the agents may go back to it"). `/v2` now redirects to `/builder`. STILL ON DISK, flagged for an explicit decision: `/edit/[slug]` + `/builder/ide` (legacy Sandpack-based surfaces — they keep `SandpackPreview`/`MonacoEditor`/`ChatPanel` etc. alive) and `/share/[code]` (keeps `EscapeHatchPreview` alive; that component carries the srcdoc-vendor-URL hotfix). Do NOT rebuild any in-browser compilation path without Craig's explicit written reversal of this rule.

39. **CRONTECH IS RENAMED VAPRON — AND NO THIRD-PARTY NAMES IN MARKETING SURFACES** (added 2026-06-10). Craig: *"we own a platform called Vapron which was once called crontech and this platform still refers to it as chrontech."* All customer-visible copy says **Vapron** (sweep completed 2026-06-10: every standalone "Crontech" word in src/ replaced). Internal identifiers stay unchanged for now (env vars `CRONTECH_PAT`/`CRONTECH_API_BASE`, `/api/crontech/*` routes, `crontech-sync.ts`, type names like `CrontechSyncInput`) — renaming those requires coordinating Vercel env config and is a separate migration; do it deliberately, not via sed. Related rule from the same session: marketing surfaces (brand bands, trust strips, tickers) must NEVER name third-party platforms or vendors (no "Vercel", "Stripe", "Neon", "Cloudflare", "Claude Opus" in customer-facing bands) — the homepage red band now carries OUR product promises only.
