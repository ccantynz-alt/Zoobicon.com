...
|---|-------|-------|---------------|
| 28 | **Builder shipping Haiku output despite rule 4 ("Opus for builds, non-negotiable")** — Audit on 2026-04-26 found `/api/generate/react-stream` (the active builder pipeline) was using `claude-haiku-4-5` for component customisation. Hidden inside a `mode === "premium" ? Sonnet : Haiku` ternary. The 7-agent Opus pipeline in `src/lib/agents.ts` was dead code — not called by the builder UI. So every "best AI builder on the market" demo was actually a Haiku build. Direct violation of rule 4. Craig: "we are trying to build the best AI website builder and video maker on the market I don't know how we can do that with using the wrong agents really." | 2026-04-26 | (1) `react-stream/route.ts`: renamed `MODEL_HAIKU` → `MODEL_PLANNER` (Haiku stays for JSON classification only), added `MODEL_DEVELOPER = "claude-opus-4-7"` and `MODEL_PREMIUM = "claude-opus-4-7"`. Both default and premium tiers now ship Opus. (2) `react/route.ts`: removed the `isPremium ? Sonnet : Haiku` branch — Opus 4.7 is the default. (3) `edit/route.ts`: replaced Haiku → Sonnet → Sonnet failover with Sonnet 4.6 → Opus 4.7 chain (diffs stay on Sonnet for speed; complex multi-file edits escalate to Opus). (4) **THE FLYWHEEL GUARD** — built `scripts/check-models.js` that scans every developer-agent route for forbidden Haiku patterns: variable assignments (`MODEL_DEVELOPER = "claude-haiku..."`), tier-conditional ternaries (the exact pattern that hid the regression), and direct `messages.create({ model: "claude-haiku..." })` SDK calls. Wired into `npm run build` (prebuild script) and CI (`.github/workflows/ci.yml`). Verified the guard catches the regression pattern by replaying it on a fixture. From this commit forward, no future merge can silently downgrade the developer agent — the build fails with a pointer back to rule 4. Procedure, not value. |
