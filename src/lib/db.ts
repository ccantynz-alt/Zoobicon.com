...
export async function initSchema() {
  ...
  // ---- Immigration Compliance Engine tables ----

  await sql`
    CREATE TABLE IF NOT EXISTS immigration_matters (
      id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      user_email          TEXT NOT NULL,
      ...
      offered_salary      JSONB NOT NULL DEFAULT '{}',
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS immigration_matters_user_email_idx ON immigration_matters (user_email)`;
  await sql`CREATE INDEX IF NOT EXISTS immigration_matters_status_idx ON immigration_matters (status)`;

  await sql`
    CREATE TABLE IF NOT EXISTS immigration_assessments (
      id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      matter_id           UUID NOT NULL REFERENCES immigration_matters(id) ON DELETE CASCADE,
      jurisdiction        TEXT NOT NULL,
      visa_category       TEXT NOT NULL,
      eligible            BOOLEAN NOT NULL DEFAULT false,
      confidence          INTEGER NOT NULL DEFAULT 0,
      rfe_risk            TEXT NOT NULL DEFAULT 'medium',
      reasoning_trail     JSONB NOT NULL DEFAULT '[]',
      compliance_flags    JSONB NOT NULL DEFAULT '[]',
      wage_compliance     JSONB NOT NULL DEFAULT '{}',
      occupation_mapping  JSONB NOT NULL DEFAULT '{}',
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS immigration_assessments_matter_id_idx ON immigration_assessments (matter_id)`;

  await sql`
    CREATE TABLE IF NOT EXISTS immigration_audit_trail (
      id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      matter_id               UUID NOT NULL REFERENCES immigration_matters(id) ON DELETE CASCADE,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS immigration_audit_trail_matter_id_idx ON immigration_audit_trail (matter_id)`;
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS immigration_documents_matter_id_idx ON immigration_documents (matter_id)`;

  await sql`
    CREATE TABLE IF NOT EXISTS immigration_regulatory_updates (
      id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      jurisdiction            TEXT NOT NULL,
      source                  TEXT NOT NULL,
      summary                 TEXT NOT NULL,
      impact_level            TEXT NOT NULL DEFAULT 'minor',
      affected_visa_categories TEXT[] NOT NULL DEFAULT '{}',
      action_required         TEXT NOT NULL DEFAULT '',
      published_date          DATE NOT NULL DEFAULT CURRENT_DATE,
      ingested_at             TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `;
  await sql`CREATE INDEX IF NOT EXISTS immigration_reg_updates_jurisdiction_idx ON immigration_regulatory_updates (jurisdiction)`;

  await sql`
    CREATE TABLE IF NOT EXISTS sponsor_compliance (
      id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      matter_id       UUID REFERENCES immigration_matters(id) ON DELETE SET NULL,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS sponsor_compliance_sponsor_idx ON sponsor_compliance (sponsor_name)`;
}
