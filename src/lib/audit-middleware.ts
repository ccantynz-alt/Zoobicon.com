 ...
 *
 * Rule 31 — audit logs delegated to Vapron platform-wide audit log.
 ...
 * Vapron once we wire its audit ingest endpoint.
 */
...
export async function auditLog(_entry: AuditEntry): Promise<void> {
  // No-op. When Vapron audit ingest is wired, replace with
