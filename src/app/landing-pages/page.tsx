...
const TEMPLATES = [
  { id: 1, name: 'SaaS Launch Pro', category: 'SaaS', conversion: '4.8%', views: '12.4K', thumb: 'from-stone-600 to-stone-600', rating: 4.9 },
  { id: 2, name: 'Webinar Signup', category: 'Webinar', conversion: '6.2%', views: '8.7K', thumb: 'from-stone-600 to-stone-600', rating: 4.8 },
  { id: 3, name: 'Ebook Download', category: 'Ebook', conversion: '7.1%', views: '15.2K', thumb: 'from-stone-600 to-stone-600', rating: 4.7 },
  { id: 4, name: 'Product Hunt Launch', category: 'Product Launch', conversion: '5.5%', views: '9.1K', thumb: 'from-stone-600 to-stone-600', rating: 4.9 },
  { id: 5, name: 'Mobile App Promo', category: 'App Download', conversion: '3.9%', views: '6.3K', thumb: 'from-stone-600 to-stone-600', rating: 4.6 },
  { id: 6, name: 'Coming Soon Hype', category: 'Coming Soon', conversion: '8.3%', views: '20.1K', thumb: 'from-stone-600 to-stone-600', rating: 4.9 },
  { id: 7, name: 'Conference 2026', category: 'Event', conversion: '4.2%', views: '5.8K', thumb: 'from-stone-600 to-stone-600', rating: 4.5 },
  { id: 8, name: 'Agency Portfolio', category: 'Agency', conversion: '3.1%', views: '7.4K', thumb: 'from-stone-600 to-stone-600', rating: 4.7 },
  { id: 9, name: 'SaaS Pricing', category: 'SaaS', conversion: '5.7%', views: '11.3K', thumb: 'from-stone-600 to-stone-600', rating: 4.8 },
  { id: 10, name: 'Free Trial Signup', category: 'SaaS', conversion: '6.8%', views: '14.6K', thumb: 'from-stone-600 to-stone-600', rating: 4.9 },
  { id: 11, name: 'Webinar Replay', category: 'Webinar', conversion: '5.4%', views: '4.2K', thumb: 'from-stone-600 to-stone-600', rating: 4.4 },
  { id: 12, name: 'Lead Magnet', category: 'Ebook', conversion: '9.2%', views: '18.7K', thumb: 'from-stone-600 to-stone-600', rating: 5.0 },
];
    ...
    <div className="min-h-screen bg-[#0a0a12] text-white">
      <header className="border-b border-white/10 bg-[#0a0a12]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => alert('Deploy functionality coming soon')} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-stone-600 to-stone-600 hover:opacity-90 transition text-sm font-medium">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Active Pages', value: '24', icon: Layout, color: 'text-stone-400' },
            { label: 'Total Views', value: '142.8K', icon: Eye, color: 'text-stone-400' },
            { label: 'Avg Conversion', value: '5.4%', icon: Target, color: 'text-stone-400' },
            { label: 'Leads Captured', value: '7,293', icon: Users, color: 'text-stone-400' },
          ].map((s) => (
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
              <div className="flex gap-2 overflow-x-auto pb-2">
                {TEMPLATE_CATEGORIES.map((cat) => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition ${selectedCategory === cat ? 'bg-stone-600' : 'bg-white/5 hover:bg-white/10'}`}>
                    {cat}
                  </button>
                ))}
              </div>
              <div className="relative">
                ...
                <input type="text" placeholder="Search templates..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-stone-500/50" />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTemplates.map((t) => (
                <div key={t.id} className="group rounded-2xl border border-white/10 bg-white/5 overflow-hidden hover:border-stone-500/30 transition">
                  <div className={`h-40 bg-gradient-to-br ${t.thumb} relative`}>
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3">
                      ...
                      <button onClick={() => {}} className="px-4 py-2 rounded-lg bg-stone-600 text-white text-sm font-medium flex items-center gap-2"><Copy className="w-4 h-4" /> Use</button>
                    </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="p-8 rounded-2xl bg-gradient-to-r from-stone-500/10 to-stone-500/10 border border-stone-500/20">
              ...
              <div className="space-y-4">
                ...
                <button onClick={handleGenerate} disabled={generating} className="w-full py-3.5 rounded-xl bg-gradient-to-r from-stone-600 to-stone-600 font-medium hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        {activeTab === 'ab-testing' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => {}} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-stone-600 text-sm font-medium hover:bg-stone-500 transition">
            ...
            {AB_TESTS.map((test) => (
              <div key={test.id} className="p-6 rounded-2xl bg-white/5 border border-white/10">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${test.status === 'running' ? 'bg-stone-500/20 text-stone-400' : 'bg-stone-500/20 text-stone-400'}`}>
                    {test.status}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className={`p-4 rounded-xl border ${test.winner === 'A' ? 'border-stone-500/30 bg-stone-500/5' : 'border-white/10 bg-white/[0.02]'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Variant A</span>
                      {test.winner === 'A' && <span className="text-xs text-stone-400 flex items-center gap-1"><Trophy className="w-3 h-3" /> Winner</span>}
                    </div>
                    <p className="text-xs text-white/50 mb-2">&ldquo;{test.variant_a}&rdquo;</p>
                    <p className="text-2xl font-bold">{test.conv_a}%</p>
                  </div>
                  <div className={`p-4 rounded-xl border ${test.winner === 'B' ? 'border-stone-500/30 bg-stone-500/5' : 'border-white/10 bg-white/[0.02]'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium">Variant B</span>
                      {test.winner === 'B' && <span className="text-xs text-stone-400 flex items-center gap-1"><Trophy className="w-3 h-3" /> Winner</span>}
                    </div>
                ...
                <div key={p.page} className="p-5 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-stone-400">{p.conversions.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-white/40">Rate</span>
                      <span className="text-stone-400 font-medium">{p.rate}%</span>
                    </div>
                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-stone-500 to-stone-500 rounded-full" style={{ width: `${p.rate * 10}%` }} />
                    </div>
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'popups' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              {[
                { name: 'Exit Intent', desc: 'Shows when user moves to close tab', trigger: 'Exit intent', color: 'from-stone-500 to-stone-500' },
                { name: 'Scroll Trigger', desc: 'Appears after scrolling 50%', trigger: 'Scroll depth', color: 'from-stone-500 to-stone-500' },
                { name: 'Time Delay', desc: 'Shows after 10 seconds', trigger: 'Timer', color: 'from-stone-500 to-stone-500' },
                { name: 'Click Trigger', desc: 'Opens on button click', trigger: 'Click event', color: 'from-stone-500 to-stone-500' },
              ].map((popup) => (
                <div key={popup.name} className="p-4 rounded-xl bg-white/5 border border-white/10 hover:border-stone-500/30 transition cursor-pointer">
                  <div className="flex items-center gap-3">
            ...
            <div className="p-1 rounded-2xl bg-gradient-to-r from-stone-500/20 to-stone-500/20">
              <div className="bg-[#0f0f1a] rounded-xl p-8 min-h-[400px] flex items-center justify-center relative">
                ...
                <div className="relative z-10 bg-white rounded-2xl p-6 max-w-sm text-black">
                  ...
                  <button onClick={() => {}} className="w-full bg-stone-600 text-white rounded-lg py-2.5 text-sm font-medium">Claim My Discount</button>
