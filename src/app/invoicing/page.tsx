...
export default function InvoicingPage() {
  ...
  const invoiceStatusColor = (s: string) => {
    if (s === "paid" || s === "accepted") return "bg-stone-500/20 text-stone-400";
    if (s === "sent") return "bg-stone-500/20 text-stone-400";
    if (s === "overdue" || s === "declined") return "bg-stone-500/20 text-stone-400";
    return "bg-white/10 text-white/40";
  };

  const STAT_CARDS = [
    { label: "Outstanding", value: formatCurrency(stats.outstanding), icon: Clock, color: "from-stone-500 to-stone-600", valueColor: "text-stone-400" },
    { label: "Paid This Month", value: formatCurrency(stats.paidThisMonth), icon: CheckCircle2, color: "from-stone-500 to-stone-600", valueColor: "text-stone-400" },
    { label: "Overdue", value: formatCurrency(stats.overdue), icon: AlertTriangle, color: "from-stone-500 to-stone-600", valueColor: "text-stone-400" },
    { label: "Total Revenue", value: formatCurrency(stats.totalRevenue), icon: DollarSign, color: "from-stone-500 to-stone-600", valueColor: "text-white" },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 py-16 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2.5 rounded-xl bg-gradient-to-br from-stone-500 to-stone-600"><Receipt className="w-6 h-6" /></div>
              <span className="text-sm font-medium text-white/50 uppercase tracking-wider">Business OS</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold mb-3">
              <span className="bg-gradient-to-r from-stone-400 via-stone-400 to-stone-400 bg-clip-text text-transparent">AI Invoicing & Proposals</span>
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-stone-400" /> AI Invoice Creator
          </h3>
          ...
          <textarea
            ...
            className="w-full h-24 bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white placeholder:text-white/30 resize-none focus:outline-none focus:ring-2 focus:ring-stone-500/50"
          />
          ...
          <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
            <motion.div variants={fadeUp} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
                ...
                <button onClick={() => {}} className="text-sm text-stone-400 hover:text-stone-300 flex items-center gap-1"><Plus className="w-4 h-4" /> New Invoice</button>
              </div>
              ...
              <div className="space-y-3">
                {payments.map((p) => (
                  <div key={p.id} className="flex items-center gap-4 py-3 border-b border-white/5 last:border-b-0">
                    <div className="p-2 rounded-lg bg-stone-500/10"><CreditCard className="w-4 h-4 text-stone-400" /></div>
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-stone-400">{formatCurrency(p.amount)}</p>
        ...
        {tab === "clients" && (
          <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-4">
            <div className="flex justify-end">
              <button onClick={() => {}} className="text-sm text-stone-400 hover:text-stone-300 flex items-center gap-1"><Plus className="w-4 h-4" /> Add Client</button>
            </div>
            {clients.map((c) => (
              <motion.div key={c.id} variants={fadeUp} className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-stone-500 to-stone-600 flex items-center justify-center font-bold text-sm">
          ...
          <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-4">
            {recurring.map((r) => (
              <motion.div key={r.id} variants={fadeUp} className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="p-2.5 rounded-lg bg-stone-500/10"><RotateCcw className="w-5 h-5 text-stone-400" /></div>
                  <div>
                ...
                <div className="flex items-center gap-6 text-sm">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${r.active ? "bg-stone-500/20 text-stone-400" : "bg-white/10 text-white/40"}`}>
