...
const TEMPLATES: PopupTemplate[] = [
  { id: 't1', name: 'Newsletter Signup', category: 'Lead Gen', type: 'exit-intent', conversionRate: 4.2, preview: { bg: 'from-stone-600 to-stone-700', headline: 'Get Weekly AI Insights', cta: 'Subscribe Free', ctaColor: 'bg-white text-stone-700' } },
  { id: 't2', name: '10% Discount', category: 'E-Commerce', type: 'exit-intent', conversionRate: 6.8, preview: { bg: 'from-stone-600 to-stone-700', headline: 'Wait! 10% Off Your First Order', cta: 'Claim Discount', ctaColor: 'bg-white text-stone-700' } },
  { id: 't3', name: 'Webinar Registration', category: 'Events', type: 'timed', conversionRate: 3.1, preview: { bg: 'from-stone-600 to-stone-700', headline: 'Free Webinar: AI in 2026', cta: 'Save My Spot', ctaColor: 'bg-stone-400 text-stone-900' } },
...
export default function PopupsPage() {
  ...
  const [editorBg, setEditorBg] = useState('from-stone-600 to-stone-700');
  ...
  return (
    <div className="min-h-screen text-white">
      <header className="border-b border-white/10 bg-[var(--paper)]/80 backdrop-blur-md sticky top-0 z-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Popups That <span className="bg-gradient-to-r from-stone-400 to-stone-400 bg-clip-text text-transparent">Actually Convert</span>
          </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
          {POPUP_TYPES.map(t => (
            <button key={t.id} onClick={() => { setSelectedType(t.id); setActiveTab('templates'); }}
              className={`p-3 rounded-xl border text-center transition-all ${selectedType === t.id ? 'border-stone-500/50 bg-stone-500/10' : 'border-white/10 bg-white/5 hover:border-white/20'}`}>
              <t.icon className="w-5 h-5 mx-auto mb-1 text-stone-400" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {filteredTemplates.map(t => (
              <button key={t.id} onClick={() => handleSelectTemplate(t)}
                className="text-left rounded-xl border border-white/10 bg-white/5 overflow-hidden hover:border-stone-500/30 transition-all group">
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-stone-400">{t.conversionRate}% CVR</span>
                  </div>
        ...
        {activeTab === 'editor' && (
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <button onClick={() => {}} className="w-full py-2.5 rounded-lg bg-gradient-to-r from-stone-600 to-stone-600 font-medium text-sm hover:opacity-90 transition-opacity">Save & Publish</button>
            </div>
            {/* Preview */}
          ...
          <div className="max-w-2xl space-y-4">
            ...
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-white/10 bg-white/5">
                ...
                <div className={`w-10 h-5 rounded-full ${rule.enabled ? 'bg-stone-500' : 'bg-white/20'}`}>
        ...
        {activeTab === 'ab-test' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => {}} className="px-4 py-2 rounded-lg bg-stone-600 text-sm font-medium hover:bg-stone-500 transition-colors flex items-center gap-1"><Sparkles className="w-4 h-4" /> AI Suggest Test</button>
            </div>
            {AB_TESTS.map(test => (
              <div key={test.id} className="p-5 rounded-xl border border-white/10 bg-white/5">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-medium">{test.name}</h4>
                  <span className={`px-2 py-0.5 rounded text-xs ${test.status === 'running' ? 'bg-stone-500/20 text-stone-400' : 'bg-stone-500/20 text-stone-400'}`}>{test.status}</span>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    ...
                    <div className="text-lg font-bold text-stone-400 mt-1">{test.aRate}%</div>
                  </div>
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    <div className="text-xs text-gray-400 mb-1">Variant B</div>
                    <div className="font-medium text-sm">{test.variantB}</div>
                    <div className={`text-lg font-bold mt-1 ${test.bRate > test.aRate ? 'text-stone-400' : 'text-stone-400'}`}>{test.bRate}%</div>
                  </div>
                </div>
                <div className="flex items-center gap-4 text-xs text-gray-400">
                  <span>{test.impressions.toLocaleString()} impressions</span>
                  <span>Confidence: <span className={test.confidence >= 95 ? 'text-stone-400' : test.confidence >= 80 ? 'text-stone-400' : 'text-gray-400'}>{test.confidence}%</span></span>
                  {test.confidence >= 95 && <span className="text-stone-400 font-medium">Statistically significant</span>}
                </div>
              </div>
            ))}
          </div>
              ...
              {ANALYTICS.map((a, i) => (
                <div key={i} className="p-4 rounded-xl border border-white/10 bg-white/5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-stone-400">{a.trend}</span>
                  </div>
                  <div className="flex items-center gap-6 text-sm">
                    ...
                    <div><span className="text-gray-400">Rate:</span> <span className="font-bold text-stone-400">{a.rate}%</span></div>
                  </div>
                  <div className="mt-2 h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-stone-500 to-stone-500 rounded-full" style={{ width: `${a.rate * 10}%` }} />
                  </div>
                </div>
              ))}
            </div>
