...
export default function AdminBookingPage() {
  ...
  const statusColor = (s: string) => {
    if (s === "confirmed" || s === "completed") return "bg-amber-50 text-amber-700 border border-amber-200";
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-stone-700 mt-1">
            ...
            <span
              className={`font-mono ${
                provider === "mock" ? "text-amber-600" : "text-amber-600"
              }`}
            >
              {provider || "detecting..."}
            </span>
        ...
        <button
          onClick={fetchData}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white hover:bg-stone-50 border border-stone-200 text-sm text-stone-700 transition-colors"
        >
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-white border border-stone-200 rounded-xl pl-10 pr-4 py-3 text-sm text-stone-700
                     placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-stone-500/20 focus:border-stone-300 transition-all"
        />
      </div>
        ...
        <>
          ...
          <div className="mb-8">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-stone-800">
              ...
              Appointments
              <span className="text-xs text-stone-700 bg-stone-100 px-2 py-0.5 rounded-full ml-2">
                {filteredAppointments.length} total
              </span>
            </h2>

            {filteredAppointments.length === 0 ? (
              <div className="text-center py-12 text-stone-600 rounded-2xl bg-stone-50 border border-stone-200">
              ...
              <div className="rounded-2xl bg-white border border-stone-200 overflow-hidden">
                <table className="w-full">
                  <thead>
                    <tr className="text-xs text-stone-600 uppercase tracking-wider border-b border-stone-200">
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {filteredAppointments.map((appt) => {
                      ...
                      return (
                        <tr key={appt.id} className="hover:bg-stone-50/60">
                          <td className="px-5 py-3">
                            <p className="text-sm font-medium text-stone-800">{appt.clientName}</p>
                            <p className="text-xs text-stone-600">{appt.clientEmail}</p>
                          </td>
              ...
              <div className="space-y-3">
                {services.map((svc) => (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{ backgroundColor: svc.color || "#78716c" }}
                        />
                    ...
                    {expandedService === svc.id && (
                      <div className="border-t border-stone-200 px-5 py-4 bg-stone-50/50">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p className="text-stone-600">{svc.currency}</p>
                          </div>
                          <div>
                            <p className="text-stone-600 text-xs mb-1">ID</p>
...
function StatCard({
  ...
  const colorMap: Record<string, string> = {
    indigo: "from-stone-50 to-stone-100/50 border-stone-200/60",
    cyan: "from-stone-50 to-stone-100/50 border-stone-200/60",
    emerald: "from-amber-50 to-amber-100/50 border-amber-200/60",
