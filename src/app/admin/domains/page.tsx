    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-stone-100 hover:bg-stone-200 border border-stone-200 text-sm text-stone-600 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl bg-white border border-stone-200 p-4">
          <div className="text-2xl font-bold text-stone-800">{domains.length}</div>
          <div className="text-xs text-stone-700">Total Domains</div>
        </div>
        <div className="rounded-xl bg-white border border-stone-200 p-4">
          <div className="text-2xl font-bold text-stone-600">{active}</div>
          <div className="text-xs text-stone-700">Active</div>
        </div>
        <div className="rounded-xl bg-white border border-stone-200 p-4">
          <div className="text-2xl font-bold text-stone-600">{pending}</div>
          <div className="text-xs text-stone-700">Pending</div>
        </div>
        <div className="rounded-xl bg-white border border-stone-200 p-4">
          <div className="text-2xl font-bold text-stone-600">{failed}</div>
          <div className="text-xs text-stone-700">Failed</div>
        </div>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 bg-white border border-stone-200 rounded-xl text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-stone-500/30 focus:border-stone-500 transition-shadow"
        />
      </div>
        ...
        <div className="space-y-3">
          {filtered.map(d => {
            ...
            const statusColor = d.status === "active"
              ? isExpired ? "text-red-700" : isExpiring ? "text-amber-700" : "text-amber-700"
              : d.status === "failed" ? "text-red-700" : "text-stone-600";
            const statusBg = d.status === "active"
              ? isExpired ? "bg-red-50" : isExpiring ? "bg-amber-50" : "bg-amber-50"
              : d.status === "failed" ? "bg-red-50" : "bg-stone-100";

            return (
              <div key={d.id || d.domain} className="rounded-xl bg-white border border-stone-200 p-5 hover:shadow-sm transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${d.status === "active" ? (isExpired ? "bg-red-500" : isExpiring ? "bg-amber-500" : "bg-amber-500") : d.status === "failed" ? "bg-red-500" : "bg-stone-400"}`} />
                    <h3 className="text-base font-semibold text-stone-800">{d.domain}</h3>
                    <button onClick={() => copyText(d.domain)} className="text-stone-300 hover:text-stone-500">
                  ...
                  <a href={`https://${d.domain}`} target="_blank" rel="noopener noreferrer" className="text-xs text-stone-600 hover:text-stone-600 flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
                  <div>
                    ...
                    <div className="font-medium text-stone-700 truncate">{d.user_email}</div>
                  </div>
                  <div>
                    <span className="text-stone-600">Registered</span>
                    <div className="text-stone-600">{d.registered_at ? new Date(d.registered_at).toLocaleDateString() : "—"}</div>
                  </div>
                  <div>
                    <span className="text-stone-600">Expires</span>
                    <div className="text-stone-600">{d.expires_at ? new Date(d.expires_at).toLocaleDateString() : "—"}</div>
                  </div>
                  <div>
                    <span className="text-stone-600">Privacy</span>
                    <div className="flex items-center gap-1">
                      {d.privacy_protection ? <><Shield className="w-3 h-3 text-stone-500" /> <span className="text-stone-600">On</span></> : <span className="text-stone-700">Off</span>}
                    </div>
                  </div>
                  <div>
                    <span className="text-stone-600">Auto-renew</span>
                    <div className="flex items-center gap-1">
                      {d.auto_renew ? <><Clock className="w-3 h-3 text-stone-500" /> <span className="text-stone-600">On</span></> : <span className="text-stone-700">Off</span>}
                    </div>
                  </div>
                </div>
