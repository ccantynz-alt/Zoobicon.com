    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition-shadow"
        />
      </div>

      {error && (
        <div className="rounded-xl bg-red-50 border border-red-200 p-4 mb-6 text-sm text-red-700 flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          {domains.length === 0 && (
            <Link href="/domains" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm transition-colors">
        ...
        <div className="space-y-3">
          {filtered.map(d => {
            ...
            const statusBg = d.status === "active"
              ...
              : d.status === "failed" ? "bg-red-50" : "bg-amber-50";
            ...
            return (
              <div key={d.id || d.domain} className="rounded-xl bg-white border border-slate-200 p-5 hover:shadow-sm transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${d.status === "active" ? (isExpired ? "bg-red-500" : isExpiring ? "bg-amber-500" : "bg-emerald-500") : d.status === "failed" ? "bg-red-500" : "bg-amber-500"}`} />
                    ...
                    <button onClick={() => copyText(d.domain)} className="text-slate-300 hover:text-slate-500">
                      {copied === d.domain ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1">
                      {d.privacy_protection ? <><Shield className="w-3 h-3 text-emerald-500" /> <span className="text-emerald-600">On</span></> : <span className="text-slate-500">Off</span>}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400">Auto-renew</span>
                    <div className="flex items-center gap-1">
                      {d.auto_renew ? <><Clock className="w-3 h-3 text-emerald-500" /> <span className="text-emerald-600">On</span></> : <span className="text-slate-500">Off</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Link href={`/builder?prompt=${encodeURIComponent(`Build a website for ${d.domain}`)}`}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 text-xs font-medium hover:bg-amber-100 border border-amber-200 transition-colors">
