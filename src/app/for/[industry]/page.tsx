    ...
    <div className="min-h-screen bg-[var(--paper)] text-white">
      ...
      <section className="pt-24 pb-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-1.5 text-sm text-stone-300 mb-6">
            <Star className="w-4 h-4 text-stone-400" /> Built for {data.name}
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight">
            {data.headline}
          </h1>
          <p className="text-xl text-stone-400 max-w-3xl mx-auto mb-4">
            {data.subheadline}
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {data.painPoints.map((point, i) => (
              <div key={i} className="flex items-start gap-3 p-4 bg-stone-500/5 border border-stone-500/10 rounded-xl">
                ...
                <p className="text-stone-300">{point}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-4 bg-white/[0.02]">
        <div className="max-w-3xl mx-auto text-center">
          <Quote className="w-10 h-10 text-stone-600 mx-auto mb-6" />
          <blockquote className="text-xl md:text-2xl font-medium mb-6 leading-relaxed">
            &ldquo;{data.testimonial.quote}&rdquo;
          </blockquote>
          <div className="mb-4">
            <p className="font-semibold">{data.testimonial.name}</p>
            <p className="text-sm text-stone-400">{data.testimonial.role}, {data.testimonial.company}</p>
          </div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-stone-400 mb-8">Start free. No credit card required. Your website in 60 seconds.</p>
          <Link
            href={`/builder?generator=${data.generatorType}`}
