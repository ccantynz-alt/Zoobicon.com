...
// ───────────────────────────────────────────────────────────────────────
// Wrapper-file builders — turn a set of component files into a
// Sandpack-ready React project the customer can preview + deploy.
// ───────────────────────────────────────────────────────────────────────

const SLOT_INDEX_TSX = `import React from "react";
...
const SLOT_STYLES_CSS = `/* Slot-locked editorial-light design tokens.
 * Components reference these via Tailwind \`text-[color:var(--ink)]\`
 * style utility classes. Pull-in via Sandpack's Tailwind CDN external
 * resource (configured in SandpackPreview.tsx). */
:root {
  --paper: #ffffff;
  --paper-elevated: #fcfaf3;
  --paper-bright: #fefdf7;
  --paper-input: #fbf9f0;
  --ink: #1a1a1c;
  --ink-secondary: #2a2a30;
  --ink-muted: #76767e;
  --rule: #ebe7d6;
  --rule-strong: #a8a392;
  --gold: #b8923f;
  --gold-soft: rgba(184, 146, 63, 0.14);
  --gold-glow: rgba(184, 146, 63, 0.40);
  --gold-deep: #8c6b25;
}
...
/* Editorial accent — Playfair italic served via Google Fonts. */
@import url("https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400..900;1,400..900&family=Inter:wght@300..900&display=swap");
...
/**
 * Sort the requested component ids into typical page order:
 * navbars first, footer last, hero/features/pricing in between.
 * Falls through to requested order for anything not categorisable.
 */
function orderComponents(ids: string[]): string[] {
  const categoryOf = (id: string): number => {
    if (id.includes("navbar")) return 0;
    if (id.includes("hero")) return 1;
    if (id.includes("features")) return 2;
    if (id.includes("stats")) return 3;
    if (id.includes("testimonials")) return 4;
    if (id.includes("pricing")) return 5;
    if (id.includes("cta")) return 6;
    if (id.includes("faq")) return 7;
    if (id.includes("footer")) return 99;
    return 50;
  };
  return [...ids].sort((a, b) => categoryOf(a) - categoryOf(b));
}

/**
 * Turn a list of ordered component ids into a fully-formed App.tsx
 * that imports each component from `./components/{id}` and renders
 * them top-to-bottom inside a single root fragment.
 */
function buildSlotAppFile(orderedIds: string[]): string {
...
/**
 * Generated by Zoobicon slot-locked composition.
 * Each component below is assembled from a hand-written template +
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        // ── PHASE 3.5: assemble wrapper files so Sandpack can render a complete site ──
        // Without these, filesOut has the components but no App.tsx
        // that uses them, no index.tsx entry, no styles, no
        // package.json. Sandpack renders blank. The wrapper builder
        // below produces a complete file set that boots cleanly.
        if (okCount > 0) {
          // Ordering: navbar first, footer last, hero second, the rest
          // in the order they were requested. This matches typical
          // page composition.
          const ordered = orderComponents(componentIds.filter((id) => filesOut[`components/${id}.tsx`]));
          filesOut["App.tsx"] = buildSlotAppFile(ordered);
          filesOut["index.tsx"] = SLOT_INDEX_TSX;
          filesOut["styles.css"] = SLOT_STYLES_CSS;
