    ...
    <div className="relative min-h-screen">
      ...
      <section className="relative pt-24 pb-20 overflow-hidden">
        ...
        <motion.div className="relative z-10 max-w-5xl mx-auto px-6 text-center" variants={staggerContainer} initial="hidden" animate="visible">
          <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-stone-500/10 border border-stone-500/20 mb-8">
            <Mic className="w-3.5 h-3.5 text-stone-400" /><span className="text-xs font-medium text-stone-300">AI-Powered Transcription</span>
          </motion.div>
          <motion.h1 variants={fadeInUp} className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.1] mb-6">
            Speak.{" "}<span className="bg-gradient-to-r from-stone-400 via-stone-400 to-stone-400 bg-clip-text text-transparent">It types.</span>
          </motion.h1>
          ...
          <motion.div variants={fadeInUp} className="flex flex-wrap justify-center gap-4">
            <Link href={user ? "/dictation" : "/auth/signup"} className="px-8 py-3.5 rounded-xl bg-gradient-to-r from-stone-500 to-stone-600 font-semibold hover:from-stone-400 hover:to-stone-500 transition-all flex items-center gap-2">{user ? "Open Dictation" : "Start Dictating"} <ArrowRight className="w-4 h-4" /></Link>
          ...
          <motion.div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
            {USE_CASES.map(u => (
              <motion.div key={u.title} variants={fadeInUp} className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5">
                <u.icon className="w-5 h-5 text-stone-400 mb-3" />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {PLANS.map(p => (
              <div key={p.name} className={`rounded-2xl p-6 border ${p.featured ? "bg-stone-500/5 border-stone-500/20 ring-1 ring-stone-500/10" : "bg-white/[0.02] border-white/[0.08]"}`}>
                ...
                <Link href={user ? "/dictation" : "/auth/signup"} className={`block text-center py-2.5 rounded-xl text-sm font-semibold transition-all ${p.featured ? "bg-gradient-to-r from-stone-500 to-stone-600 hover:from-stone-400 hover:to-stone-500" : "bg-white/[0.05] border border-white/[0.1] hover:bg-white/[0.08]"}`}>{user ? "Open App" : "Start Free Trial"}</Link>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 border-t border-white/[0.06]">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <Mic className="w-12 h-12 text-stone-400 mx-auto mb-6" />
          ...
          <Link href={user ? "/dictation" : "/auth/signup"} className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-stone-500 to-stone-600 font-semibold hover:from-stone-400 hover:to-stone-500 transition-all">{user ? "Open Dictation" : "Start Dictating"} <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </section>
