    ...
    <div className="min-h-screen bg-[#0a0a12] text-white">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#12121f] border border-white/10 rounded-2xl p-8 max-w-md w-full mx-4">
            ...
            <div className="space-y-4 mb-6">
              <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                ...
                <ul className="space-y-2 text-sm text-white/60">
                  <li className="flex items-center gap-2"><Check className="w-4 h-4 text-stone-400" /> Read your account information</li>
                  <li className="flex items-center gap-2"><Check className="w-4 h-4 text-stone-400" /> Send notifications on your behalf</li>
                  <li className="flex items-center gap-2"><Check className="w-4 h-4 text-stone-400" /> Access relevant project data</li>
                </ul>
              </div>
              <div>
                <label className="text-sm text-white/50 mb-1 block">API Key (optional)</label>
                <input type="password" placeholder="Enter API key..." className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-500/50" />
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={handleStartConnect} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-stone-600 to-stone-600 hover:opacity-90 transition text-sm font-medium flex items-center justify-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-stone-500/50"
            />
          </div>
          <div className="flex items-center gap-2">
            <div className="flex rounded-lg overflow-hidden border border-white/10">
              <button onClick={() => setViewMode('grid')} className={`px-3 py-2 transition ${viewMode === 'grid' ? 'bg-stone-600' : 'bg-white/5 hover:bg-white/10'}`}><Grid3X3 className="w-4 h-4" /></button>
              <button onClick={() => setViewMode('list')} className={`px-3 py-2 transition ${viewMode === 'list' ? 'bg-stone-600' : 'bg-white/5 hover:bg-white/10'}`}><List className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition ${
                selectedCategory === cat.id ? 'bg-stone-600 text-white' : 'bg-white/5 text-white/60 hover:text-white hover:bg-white/10'
              }`}
            >
        ...
        {selectedCategory === 'all' && !searchQuery && (
          <div className="mb-8">
            <h3 className="text-sm font-medium text-white/40 mb-3 flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-stone-400" /> Connected ({connectedCount})</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {INTEGRATIONS.filter(i => i.connected).map((integration) => (
                <div key={integration.id} className="p-4 rounded-xl bg-stone-500/5 border border-stone-500/20 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400 flex items-center gap-1"><Wifi className="w-3 h-3" /> Connected</p>
                  </div>
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.filter(i => selectedCategory !== 'all' || !i.connected || searchQuery).map((integration) => (
              <div key={integration.id} className={`p-5 rounded-2xl border transition hover:border-stone-500/30 ${integration.connected ? 'bg-stone-500/5 border-stone-500/20' : 'bg-white/5 border-white/10'}`}>
                <div className="flex items-start justify-between mb-3">
                  ...
                  {integration.popular && <span className="px-2 py-0.5 rounded-full bg-stone-500/20 text-stone-400 text-xs">Popular</span>}
                </div>
                ...
                {integration.connected ? (
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-stone-400 flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> Connected</span>
                  ...
                  <div className="flex items-center justify-center gap-2 text-sm text-stone-400">
                    <div className="w-4 h-4 border-2 border-stone-400/30 border-t-violet-400 rounded-full animate-spin" />
                    Connecting...
                  </div>
                ) : (
          ...
          <div className="space-y-2">
            {filtered.map((integration) => (
              <div key={integration.id} className={`flex items-center gap-4 px-5 py-4 rounded-xl border transition ${integration.connected ? 'bg-stone-500/5 border-stone-500/20' : 'bg-white/5 border-white/10'}`}>
                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold" style={{ backgroundColor: integration.color }}>
                  {integration.name[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {integration.popular && <Star className="w-3.5 h-3.5 text-stone-400 fill-stone-400" />}
                  </div>
                ...
                {integration.connected ? (
                  <span className="flex items-center gap-1 text-xs text-stone-400"><CheckCircle2 className="w-3.5 h-3.5" /> Connected</span>
                ) : (
