...
const THEMES = [
  ...
  { id: 'glass', name: 'Glass', bg: 'bg-gradient-to-b from-stone-900 to-stone-900', text: 'text-white', card: 'bg-white/10 backdrop-blur' },
...
export default function LinkBioPage() {
  ...
  const [links, setLinks] = useState<BioLink[]>([
    ...
    { id: '3', title: 'Camera', url: 'https://instagram.com/yourname', icon: '📸', featured: false },
      ...
      <header className="border-b border-white/10 bg-[#0a0a12]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2"><LinkIcon className="w-5 h-5 text-stone-400" /><span className="font-semibold">Link in Bio</span></div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => { navigator.clipboard.writeText(pageUrl); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="px-4 py-2 bg-white/10 rounded-lg text-sm flex items-center gap-2 hover:bg-white/20 transition-colors">
              {copied ? <Check className="w-4 h-4 text-stone-400" /> : <Copy className="w-4 h-4" />} {pageUrl}
            </button>
            <button onClick={() => alert('Publishing your link bio page...')} className="px-4 py-2 bg-stone-600 hover:bg-stone-500 rounded-lg text-sm font-medium transition-colors">Publish</button>
          </div>
        </div>
      </header>
        ...
        {tab === 'editor' && (
          <div className="flex gap-8">
            ...
            <div className="flex-1 space-y-4">
              {/* AI Generator */}
              <div className="bg-gradient-to-r from-stone-500/10 to-stone-500/10 border border-stone-500/20 rounded-xl p-4">
                <div className="flex gap-3">
                  <input value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Describe your brand and AI will create your link page..." className="flex-1 bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-stone-500" />
                  <button onClick={() => alert('AI link generation coming soon')} className="px-4 py-2 bg-stone-600 rounded-lg text-sm font-medium flex items-center gap-2"><Sparkles className="w-4 h-4" /> Generate</button>
                </div>
              </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs text-white/50 mb-1">Username</label><div className="flex"><span className="bg-white/10 border border-r-0 border-white/20 rounded-l-lg px-3 py-2 text-sm text-white/40">zoobicon.sh/</span><input value={username} onChange={e => setUsername(e.target.value)} className="flex-1 bg-white/10 border border-white/20 rounded-r-lg px-3 py-2 text-sm focus:outline-none focus:border-stone-500" /></div></div>
                  <div><label className="block text-xs text-white/50 mb-1">Display Name</label><input value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-stone-500" /></div>
                  <div className="col-span-2"><label className="block text-xs text-white/50 mb-1">Bio</label><textarea value={bio} onChange={e => setBio(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm h-16 resize-none focus:outline-none focus:border-stone-500" /></div>
                </div>
              </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={addLink} className="px-3 py-1.5 bg-stone-600 rounded-lg text-xs font-medium flex items-center gap-1"><Plus className="w-3.5 h-3.5" /> Add Link</button>
                </div>
                <div className="space-y-2">
                  {links.map(link => (
                    <div key={link.id} className="flex items-center gap-2 p-2.5 bg-white/5 border border-white/10 rounded-lg group">
                      ...
                      <button onClick={() => updateLink(link.id, { featured: !link.featured })} className={`p-1 rounded ${link.featured ? 'text-stone-400' : 'text-white/20'}`}>★</button>
                      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => removeLink(link.id)} className="p-1 hover:bg-stone-500/20 rounded"><Trash2 className="w-3 h-3 text-stone-400" /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mx-auto w-[280px] h-[580px] rounded-[2.5rem] border-4 border-white/20 bg-black overflow-hidden relative">
                ...
                <div className={`h-full overflow-y-auto px-5 pt-12 pb-6 ${theme.bg} ${theme.text}`}>
                  <div className="text-center mb-6">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-stone-500 to-stone-500 mx-auto mb-3 flex items-center justify-center text-2xl font-bold text-white">{displayName[0]}</div>
                    <h2 className="text-lg font-bold">{displayName}</h2>
                    <p className="text-sm opacity-70 mt-1">{bio}</p>
                  </div>
                  <div className="space-y-2.5">
                    {links.map(link => (
                      <div key={link.id} className={`${theme.card} rounded-xl p-3 text-center transition-transform hover:scale-[1.02] cursor-pointer ${link.featured ? 'ring-2 ring-stone-500' : 'border border-white/10'}`}>
        ...
        {tab === 'themes' && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {THEMES.map(t => (
                <button key={t.id} onClick={() => setSelectedTheme(t.id)} className={`rounded-xl overflow-hidden border-2 transition-all ${selectedTheme === t.id ? 'border-stone-500 scale-105' : 'border-white/10 hover:border-white/30'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-5">
                  ...
                  <div className="text-xs text-stone-400 mt-1">{s.change}</div>
                </div>
              ))}
            </div>
                ...
                return (
                  <div key={link.id} className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 bg-white/5 rounded-full h-6 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-stone-600 to-stone-600 rounded-full flex items-center justify-end px-2" style={{ width: `${(clicks / 3200) * 100}%` }}>
