...
export default function CreatorMarketplacePage() {
  ...
  const STAT_CARDS = [
    { label: "Templates", value: String(totalTemplates), icon: Package, color: "from-indigo-500 to-purple-600" },
    { label: "Total Sales", value: totalSales.toLocaleString(), icon: ShoppingCart, color: "from-emerald-500 to-teal-600" },
    { label: "Avg Rating", value: avgRating, icon: Star, color: "from-amber-500 to-orange-600" },
    { label: "Creators", value: String(new Set(templates.map((t) => t.creatorId)).size), icon: Crown, color: "from-rose-500 to-pink-600" },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 py-16 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2.5 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600"><Store className="w-6 h-6" /></div>
              <span className="text-sm font-medium text-white/50 uppercase tracking-wider">Creator Economy</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold mb-3">
              <span className="bg-gradient-to-r from-amber-400 via-orange-400 to-rose-400 bg-clip-text text-transparent">Creator Marketplace</span>
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        {view === "browse" && (
          <>
            {/* Featured Creator */}
            {featured && (
              <motion.div variants={fadeUp} initial="hidden" animate="visible" className="bg-gradient-to-br from-amber-500/10 to-orange-500/5 border border-amber-500/20 rounded-2xl p-6">
                <div className="flex items-center gap-1.5 text-xs text-amber-400 font-semibold mb-3"><Crown className="w-3.5 h-3.5" /> FEATURED CREATOR</div>
                <div className="flex items-start gap-4">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center font-bold text-xl">{featured.avatar}</div>
                  <div className="flex-1">
                    <h3 className="text-xl font-bold">{featured.name}</h3>
                    <p className="text-sm text-white/50 mt-1">{featured.bio}</p>
                    <p className="text-sm text-amber-400 mt-2">{featured.totalSales.toLocaleString()} total sales</p>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                />
              </div>
              <select
                value={sort}
              ...
              {MARKETPLACE_CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                    category === cat ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" : "bg-white/5 text-white/50 border border-white/10 hover:text-white/70"
                  }`}
                >
                  {cat}
                </button>
              ...
              {templates.map((t) => (
                <motion.div key={t.id} variants={fadeUp} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-colors group cursor-pointer">
                  <div className={`h-36 bg-gradient-to-br ${t.previewGradient} relative`}>
                    ...
                    {t.price === 0 && (
                      <span className="absolute top-3 left-3 px-2 py-0.5 bg-emerald-500 rounded text-[10px] font-bold">FREE</span>
                    )}
                  </div>
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-2 text-xs text-white/30">
                      <span className="flex items-center gap-1"><Star className="w-3 h-3 text-amber-400" /> {t.rating}</span>
          ...
          <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                ...
                <p className="text-3xl font-bold text-emerald-400">${(earnings.totalEarnings / 100).toLocaleString()}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div key={s.step} className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-sm font-bold shrink-0">{s.step}</div>
                    <div>
            ...
            <motion.div variants={fadeUp} className="text-center">
              <Link href="/builder" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-600 rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
