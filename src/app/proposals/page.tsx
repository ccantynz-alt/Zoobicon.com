...
const STATUS_CONFIG: Record<ProposalStatus, { label: string; color: string; bgColor: string; icon: React.ReactNode }> = {
  ...
  sent: { label: 'Sent', color: 'text-stone-400', bgColor: 'bg-stone-500/20', icon: <Send className="w-3.5 h-3.5" /> },
  viewed: { label: 'Viewed', color: 'text-stone-400', bgColor: 'bg-stone-500/20', icon: <Eye className="w-3.5 h-3.5" /> },
  signed: { label: 'Signed', color: 'text-stone-400', bgColor: 'bg-stone-500/20', icon: <CheckCircle className="w-3.5 h-3.5" /> },
  expired: { label: 'Expired', color: 'text-stone-400', bgColor: 'bg-stone-500/20', icon: <XCircle className="w-3.5 h-3.5" /> },
};
    ...
    <div className="min-h-screen bg-[#0a0a12] text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Proposals', value: DEMO_PROPOSALS.length, icon: <FileText className="w-5 h-5" />, color: 'text-stone-400' },
            { label: 'Pipeline Value', value: `$${totalValue.toLocaleString()}`, icon: <DollarSign className="w-5 h-5" />, color: 'text-stone-400' },
            { label: 'Won Revenue', value: `$${signedValue.toLocaleString()}`, icon: <CheckCircle className="w-5 h-5" />, color: 'text-stone-400' },
            { label: 'Pending', value: `$${pendingValue.toLocaleString()}`, icon: <Clock className="w-5 h-5" />, color: 'text-stone-400' },
          ].map((stat, i) => (
          ...
          <div>
            <div className="flex items-center gap-4 mb-6">
              <div className="relative flex-1 max-w-md">
                ...
                <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search proposals..." className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-stone-500/50" />
              </div>
              <div className="flex gap-1 bg-white/5 rounded-lg p-1">
                {(['all', 'draft', 'sent', 'viewed', 'signed', 'expired'] as const).map(status => (
                  <button key={status} onClick={() => setFilterStatus(status)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all capitalize ${filterStatus === status ? 'bg-stone-600 text-white' : 'text-white/50 hover:text-white'}`}>
                    {status}
                  </button>
                ))}
              </div>
                      ...
                      {selectedProposal?.id === proposal.id && (
                        <div className="mt-4 pt-4 border-t border-white/10">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => {}} className="flex items-center gap-1.5 px-3 py-1.5 bg-stone-600 hover:bg-stone-500 rounded-lg text-xs transition-colors"><Eye className="w-3.5 h-3.5" />Preview</button>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" placeholder="Company name" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-stone-500/50" />
                  </div>
                  <div>
                    <label className="block text-sm text-white/70 mb-1.5">Client Email</label>
                    <input type="email" placeholder="client@company.com" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-stone-500/50" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
            ...
            {TEMPLATES.map((template, i) => (
              <div key={i} className="bg-white/5 rounded-2xl border border-white/10 hover:border-stone-500/50 p-6 transition-all group cursor-pointer">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs px-2 py-0.5 rounded-full bg-stone-500/20 text-stone-300">{template.category}</span>
            ...
            <div className="bg-white/5 rounded-2xl border border-white/10 p-8">
              ...
              <div className="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
                ...
                <div className="mb-4">
                  ...
                  <input type="text" value={signatureName} onChange={e => setSignatureName(e.target.value)} placeholder="Enter your full name" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-stone-500/50" />
                </div>
                <div className="mb-4">
                  <label className="block text-sm text-white/70 mb-2">Signature</label>
                  <div
                    onClick={() => setSignatureDrawn(true)}
                    className={`h-32 rounded-xl border-2 border-dashed flex items-center justify-center cursor-pointer transition-all ${signatureDrawn ? 'border-stone-500/50 bg-stone-500/5' : 'border-white/20 hover:border-white/40 bg-white/5'}`}
                  >
                    {signatureDrawn && signatureName ? (
                      <p className="text-3xl font-serif italic text-stone-300">{signatureName}</p>
                    ) : (
                ...
                {signatureDrawn && (
                  <div className="flex items-center gap-3 p-3 bg-stone-500/10 border border-stone-500/20 rounded-xl">
                    <CheckCircle className="w-5 h-5 text-stone-400" />
                    <div>
                      <p className="text-sm font-medium text-stone-400">Signature captured</p>
      ...
      <footer className="border-t border-white/10 mt-20 py-12">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className="text-white/40 text-sm">
            Replaces PandaDoc ($49/mo), Proposify ($49/mo), and DocuSign &mdash; <span className="text-stone-400">included free with Zoobicon Pro</span>
          </p>
