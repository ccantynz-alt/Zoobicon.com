...
export default function AutomationPage() {
  ...
  const [canvasNodes, setCanvasNodes] = useState<WorkflowNode[]>([
    { id: 'n1', type: 'trigger', label: 'Form Submitted', icon: <FileText className="w-5 h-5" />, config: { form: 'Contact Page' }, color: 'border-stone-500/50 bg-stone-500/10' },
    { id: 'n2', type: 'action', label: 'Create CRM Contact', icon: <Database className="w-5 h-5" />, config: { fields: 'name, email, phone' }, color: 'border-stone-500/50 bg-stone-500/10' },
    { id: 'n3', type: 'action', label: 'Send Welcome Email', icon: <Mail className="w-5 h-5" />, config: { template: 'welcome-v2' }, color: 'border-stone-500/50 bg-stone-500/10' },
    { id: 'n4', type: 'action', label: 'Notify Team', icon: <Bell className="w-5 h-5" />, config: { channel: '#leads' }, color: 'border-stone-500/50 bg-stone-500/10' },
  ]);
    ...
    <div className="min-h-screen bg-[#0a0a12] text-white">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Workflow className="w-5 h-5 text-stone-400" />
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Active Workflows', value: DEMO_WORKFLOWS.filter(w => w.status === 'active').length, color: 'text-stone-400' },
            { label: 'Total Executions', value: '5.5K', color: 'text-stone-400' },
            { label: 'Success Rate', value: '98.7%', color: 'text-stone-400' },
            { label: 'Time Saved', value: '142 hrs', color: 'text-stone-400' },
          ].map((stat, i) => (
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => {}} className="flex items-center gap-2 px-4 py-2.5 bg-stone-600 hover:bg-stone-500 rounded-lg text-sm font-medium transition-colors">
        ...
        {activeTab === 'workflows' && (
          <div className="space-y-3">
            {DEMO_WORKFLOWS.map(workflow => {
              const statusColors = { active: 'text-stone-400 bg-stone-500/20', paused: 'text-stone-400 bg-stone-500/20', draft: 'text-white/60 bg-white/10', error: 'text-stone-400 bg-stone-500/20' };
                ...
                <div key={workflow.id} className="bg-white/5 rounded-2xl border border-white/10 hover:border-white/20 p-6 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="hidden md:block text-right">
                      <p className={`text-sm font-medium ${workflow.successRate >= 99 ? 'text-stone-400' : workflow.successRate >= 95 ? 'text-stone-400' : 'text-stone-400'}`}>{workflow.successRate}%</p>
                  ...
                  <div className="mt-4 flex items-center gap-2 text-xs">
                    <span className="px-2 py-1 bg-stone-500/10 text-stone-400 rounded-lg flex items-center gap-1"><Zap className="w-3 h-3" />{workflow.trigger}</span>
                    <ArrowDown className="w-3 h-3 text-white/20 rotate-[-90deg]" />
                    {workflow.actions.map((action, i) => (
                      <React.Fragment key={i}>
                        <span className="px-2 py-1 bg-stone-500/10 text-stone-400 rounded-lg">{action}</span>
              ...
              <div className="bg-white/5 rounded-2xl border border-white/10 p-4">
                ...
                <div className="space-y-2">
                  {TRIGGER_TYPES.map(trigger => (
                    <div key={trigger.id} className="flex items-center gap-3 p-3 bg-white/5 hover:bg-stone-500/10 border border-white/5 hover:border-stone-500/30 rounded-xl cursor-grab transition-all group">
                      <span className="text-stone-400">{trigger.icon}</span>
                      <div>
              ...
              <div className="bg-white/5 rounded-2xl border border-white/10 p-4">
                ...
                <div className="space-y-2">
                  {ACTION_TYPES.map(action => (
                    <div key={action.id} className="flex items-center gap-3 p-3 bg-white/5 hover:bg-stone-500/10 border border-white/5 hover:border-stone-500/30 rounded-xl cursor-grab transition-all group">
            ...
            <div className="lg:col-span-3 bg-white/5 rounded-2xl border border-white/10 p-8 min-h-[500px]">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                  <button onClick={() => {}} className="px-3 py-1.5 bg-stone-600 hover:bg-stone-500 rounded-lg text-xs font-medium transition-colors flex items-center gap-1"><Play className="w-3 h-3" />Test Run</button>
                  <button onClick={() => {}} className="px-3 py-1.5 bg-stone-600 hover:bg-stone-500 rounded-lg text-xs font-medium transition-colors">Save</button>
                </div>
              </div>
              <div className="flex flex-col items-center gap-4">
                ...
                <button onClick={() => {}} className="w-full max-w-md border-2 border-dashed border-white/20 hover:border-stone-500/50 rounded-2xl p-5 text-center text-sm text-white/30 hover:text-stone-400 transition-all flex items-center justify-center gap-2">
        ...
        {activeTab === 'logs' && (
          <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {DEMO_LOGS.map(log => {
                const statusConfig = {
                  success: { icon: <CheckCircle className="w-4 h-4 text-stone-400" />, color: 'text-stone-400' },
                  failure: { icon: <XCircle className="w-4 h-4 text-stone-400" />, color: 'text-stone-400' },
                  running: { icon: <div className="w-4 h-4 border-2 border-stone-400 border-t-transparent rounded-full animate-spin" />, color: 'text-stone-400' },
                };
                  ...
                  <div key={log.id} className="px-6 py-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
                    ...
                    <div className="hidden md:block text-right">
                      ...
                      <div className="w-20 h-1.5 bg-white/10 rounded-full mt-1 overflow-hidden">
                        <div className={`h-full rounded-full ${log.status === 'success' ? 'bg-stone-500' : log.status === 'failure' ? 'bg-stone-500' : 'bg-stone-500'}`} style={{ width: `${(log.stepsCompleted / log.totalSteps) * 100}%` }} />
                      </div>
                    </div>
      ...
      <footer className="border-t border-white/10 mt-20 py-12">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className="text-white/40 text-sm">
            Replaces Zapier ($20/mo), Make ($9/mo), and n8n &mdash; <span className="text-stone-400">included free with Zoobicon Pro</span>
          </p>
