...
export default function EmailSupportDashboard() {
  ...
  const handleCreateTicket = () => {
    ...
    const ticket: Ticket = {
      ...
      messages: [
        { id: `m-${Date.now()}`, from: "customer", body: newBody, timestamp: new Date().toISOString() },
      ],
      tags: [],
    };
                  ...
                  >
                    ...
                    {msg.from !== "customer" && (
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                        msg.from === "ai" ? "bg-purple-500/20" : "bg-blue-500/20"
                      }`}>
                        {msg.from === "ai" ? (
              ...
              <div className="px-6 py-3 border-t border-white/[0.06] shrink-0">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-xs text-white placeholder:text-white/25 outline-none focus:border-blue-500/30 transition-colors"
                  />
                  <button
                    onClick={handleSendReply}
                    disabled={!replyText.trim()}
                    className="p-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white transition-all disabled:opacity-30"
                  >
