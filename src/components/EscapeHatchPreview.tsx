  ...
  const runtime = String.raw`
    ...
    (async () => {
      try {
        ...
        // identical to the previous build. We cache by a fast hash of
        ...
        const CACHE_PREFIX = "zoobicon:transpile:v3:";
        const CACHE_MAX = 200; // hard cap on cached entries — LRU sweep below
        // CRITICAL: do NOT use crypto.subtle here. window.crypto.subtle
        // is unavailable inside srcdoc / opaque-origin iframes on iOS
        // Safari (and some other WebKit contexts), so calling
        // crypto.subtle.digest throws SYNCHRONOUSLY — which crashed the
        // ENTIRE preview on iPad before a single component rendered
        // ("Preview failed", every build, the months-long bug). A cache
        // key needs no cryptographic strength: a collision just causes a
        // harmless recompile. FNV-1a + length, computed synchronously,
        // can never throw and works in every browser context.
        // (No backticks in this comment — it lives inside a String.raw
        //  template, where a backtick would terminate the string.)
        function hashKey(s) {
          try {
            let h = 0x811c9dc5;
            ...
            // Never let key derivation break the build.
            return "nohash-" + s.length;
          }
        }
        ...
        async function compileModule(path) {
          ...
          const cacheKey = hashKey(path + "\0" + sourceForBabel);
